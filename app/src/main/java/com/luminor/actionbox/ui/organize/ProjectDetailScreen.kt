...
fun ProjectDetailScreen(viewModel: ActionViewModel, projectId: Long, onBack: () -> Unit) {
    ...
    val tasks = all.filter { it.projectId == projectId }.sortedBy { it.createdAt }
        ...
        Column(
            ...
            if (editing) {
                ...
                ActionCard {
                    Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        if (draftTasks.isEmpty()) {
                            Text("Nenhuma tarefa. Adicione a primeira abaixo.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                        draftTasks.forEachIndexed { index, draft ->
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    Button(
                        onClick = {
                            val existingTitles = draftTasks.mapNotNull { draft ->
                                draft.id?.let { id -> id to draft.title.trim() }
                            }.toMap()
                ...
                ActionCard {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        if (tasks.isEmpty()) {
                            Text("Nenhuma tarefa ainda. Toque em Editar para adicionar.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                        }
                        tasks.forEach { task ->
                            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
    ...
    if (showDiscardDialog) {
        AlertDialog(
            ...
            text = { Text("As alterações feitas no título, descrição e tarefas não serão salvas.") },
            confirmButton = {
                TextButton(onClick = {
    ...
    if (showDeleteDialog) {
        AlertDialog(
            ...
            text = { Text("O projeto e todas as tarefas vinculadas serão excluídos permanentemente.") },
            confirmButton = {
                TextButton(onClick = {
